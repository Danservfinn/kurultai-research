...
function wrapLines(value: string, maxLength: number): string[] {
  const words = value.split(/\s+/);
  const lines: string[] = [];
  for (const word of words) {
    const current = lines[lines.length - 1];
    if (!current || (current.length + 1 + word.length > maxLength && lines.length < 2)) lines.push(word);
...
export function renderKnowledgeGraphSvg(post: PublicPost): string {
  ...
  const titleLines = wrapLines(post.title, 72);
  const paperNodeHeight = titleLines.length > 1 ? 116 : 94;
  const legendY = synthesisY + paperNodeHeight + 48;
  ...
  const paperTitleMarkup = titleLines.map((line, index) => `<tspan x="366" y="${synthesisY + 57 + index * 24}">${escapeXml(line)}</tspan>`).join("");
  const paperDetailY = synthesisY + (titleLines.length > 1 ? 102 : 78);
