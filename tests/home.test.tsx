...
describe("research index", () => {
  it("states the editorial and authority boundary", () => {
    ...
    expect(screen.getByRole("link", { name: /read the latest synthesis/i })).toHaveAttribute("href", "/research/parsethis-buyer-pain-miner-from-x-security-research-2218/");
    expect(screen.getByRole("link", { name: /open agent interface/i })).toHaveAttribute("href", "/agents/");
    expect(screen.getByText(/11 papers/i)).toBeInTheDocument();
  });
});
